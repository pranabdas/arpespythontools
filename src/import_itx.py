...
def import_itx(data_path):
    ...
    if (contents[1][:8] == 'WAVES/D\t' or contents[1][:8] == 'WAVES/D '):
        ...
        x = np.linspace(
            dimoffset, (dimoffset + dimdelta*(dimsize - 1)), dimsize)

        return data, x

    # Higher dimensional data file
    else:
        dimsize = (contents[1][contents[1].find(
            '(')+1: contents[1].find(')')]).split(',')
        ...
        if (len(dimsize) == 2):
            ...
            for ii in range(dimsize[0]):
                data_row = contents[ii+3][contents[ii +
                                                   3].find('\t')+1:contents[ii+3].find('\n')]
                data_row = data_row.split('\t')
                data[ii, :] = np.asarray(list(map(float, data_row)))

            temp = contents[4+dimsize[0]].split(';')
            x_offset = float(temp[0][temp[0].find(
                'x')+1:temp[0].find('"')].split(',')[:-1][0])
            x_delta = float(temp[0][temp[0].find(
                'x')+1:temp[0].find('"')].split(',')[:-1][1])
            y_offset = float(temp[1][temp[1].find(
                'y')+1:temp[1].find('"')].split(',')[:-1][0])
            y_delta = float(temp[1][temp[1].find(
                'y')+1:temp[1].find('"')].split(',')[:-1][1])
        ...
        # 3-dimensional data
            ...
            data = data.reshape(
                (dimsize[1], dimsize[0], dimsize[2]), order='F')
            ...
            y_delta = float(temp[1][temp[1].find(
                'y')+1:temp[1].find('"')].split(',')[:-1][1])
            z_offset = float(temp[2][temp[2].find(
                'z')+1:temp[2].find('"')].split(',')[:-1][0])
            z_delta = float(temp[2][temp[2].find(
                'z')+1:temp[2].find('"')].split(',')[:-1][1])
