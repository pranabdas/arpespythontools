...
def rotate(data, angle, x, y):
    ...
    data_temp = np.zeros([int(abs(data.shape[1]*np.cos(angle_rad)) + \
    ...
    X = np.linspace(X_start, X_end, data_temp.shape[0])
    Y = np.linspace(Y_start, Y_end, data_temp.shape[1])
    ...
    for ii in range(data.shape[0]):
        ...
        for jj in range(data_temp.shape[0]):
            for kk in range(data_temp.shape[1]):
                x_temp, y_temp = rot_matrix @[[X_grid[jj, kk]], [Y_grid[jj, kk]]]

                if x_temp < x_start or x_temp > x_end or \
                y_temp < y_start or y_temp > y_end:
                    data_temp[jj, kk] = np.nan
                else:
                    data_temp[jj, kk] = interp_func(x_temp, y_temp)

        data_r.append(data_temp)

    data_r = np.vstack(data_temp)
    return data_r, X, Y
